...
/* ----------------------------------- adding options object to dependency of useOnRepetition  -----------------------------------  */
...
/* export const useBalance = (provider, address, pollTime = 0) => {
  ...
  useOnRepetition(
    pollBalance,
    { pollTime, provider, leadingTrigger: address != null && address !== "" && provider != null, address },
    provider,
    address,
  );
...
export const useOnRepetition = (callback, options, ...args) => {
  ...
  useEffect(() => {
    ////////// Remove leadingCall ref => NOT SURE ABOUT SIDE EFFECTS
    if (options.leadingTrigger && callFunctionWithArgs != null) {
      callFunctionWithArgs();
    }
    ////////// Add options (vs options.leadingTrigger) to run this on any options change
  }, [options, callFunctionWithArgs]);
}; */
...
/* ----------------------------------- adding new useEffect for change in address ----------------------------------- */
...
export const useOnRepetition = (callback, options, ...args) => {
  ...
  const argDeps = JSON.stringify(args !== null && args !== void 0 ? args : []);
  const argsAddress = args !== null && args !== void 0 && args.length >= 2 ? args[1] : "";
  ...
  // call if triggered by extra watch, however only on inital call
  useEffect(() => {
    console.log(`Inside only at start useEffect !!!!`);
    if (
...
/* ----------------------------------- adding new useEffect with argsDeps  ----------------------------------- */
